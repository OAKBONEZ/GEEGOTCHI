...
"""
SPI TFT display test (LCDwiki MPI2801 or Waveshare-style ST7789).

LCDwiki 2.8" RPi Display (SKU MPI2801, ILI9341): Pi header pin 15 = GPIO22 (DC/LCD_RS),
pin 13 = GPIO27 (RST). See https://www.lcdwiki.com/2.8inch_RPi_Display

Waveshare-style 2.4"/2.8" ST7789 stack: DC=GPIO25, RST=GPIO27.

Switch with env: DISPLAY_TEST_PROFILE=lcdwiki_mpi2801 | waveshare_st7789
(Default in this repo: lcdwiki_mpi2801 — matches setup-fancygotchi.sh DC patch.)

If GPIO27 (RST) cannot be claimed: DISPLAY_TEST_NO_RST=1 (panel may still work after power-on reset.)
"""
...
DISPLAY_PROFILE = os.environ.get("DISPLAY_TEST_PROFILE", "lcdwiki_mpi2801").strip().lower()
...
print_status('ℹ', f"DISPLAY_TEST_PROFILE={DISPLAY_PROFILE!r} (lcdwiki_mpi2801 | waveshare_st7789)")
        ...
        for p in pins:
            try:
                ...
                print_status('ℹ', "If LCD-show / fbtft / a dtoverlay drives this display, the kernel "
                                  ...
                                  "or use Pi 4 + pwnagotchi without the panel framebuffer overlay.")
                print_status('ℹ', "Also run: cat /proc/device-tree/model | tr -d '\\0'")
                return False
            configured.append(p)
...
try:
    ...
    if DISPLAY_PROFILE in ("lcdwiki_mpi2801", "mpi2801", "lcdwiki"):
        ...
        print_status('ℹ', "Use: lcdwiki_mpi2801 | waveshare_st7789")
        sys.exit(1)
...
print("\nNext steps:")
print("  1. MPI2801: sudo bash setup-fancygotchi.sh")
print("  2. Waveshare stack: RPI4BFANCY/pwnagotchi_fancygotchi_setup.sh on the Pi")
